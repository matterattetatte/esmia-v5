...
export default async function(eleventyConfig) {
	...
	// Filter to exclude unwanted tags (e.g., "post")
	eleventyConfig.addFilter("filterTagList", tags => {
		return tags.filter(tag => !["post", "all"].includes(tag));
	});

eleventyConfig.addFilter("findByUrl", (collection, url) => {
    return collection.find(item => {
      return item.url === url || (item.data.eleventyNavigation && item.data.eleventyNavigation.url === url);
    });
  });
